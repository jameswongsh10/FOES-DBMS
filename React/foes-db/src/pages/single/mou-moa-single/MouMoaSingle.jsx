    ...
    const generateForm = (obj) => {
        ...
        for (const key in obj) {
            if (!(key == 'id' || key == 'created_at' || key == 'updated_at')) {
                if (key == 'isActive') {
                    formHtml.push(
                        <Input name={key} hint=" (Yes or No)" key={key} initialValue={obj[key]}
                               onFormChangeHandler={onFormChangeHandler}/>
                    );
                } else if (key == 'signed_date') {
                    formHtml.push(
    ...
    const generateSection = (arr) => {
        ...
        for (var i = 0; i < arr.length; i++) {
            if (keyPersons[i] === null) {
                sectionHtml.push(
                    <KeyPersonSection obj={null} index={i} keyPersons={keyPersons} setKeyPersons={setKeyPersons}
                                      mouID={id} key={`temp${i}`}/>
                );
            } else {
                sectionHtml.push(
                    <KeyPersonSection obj={keyPersons[i]} index={i} keyPersons={keyPersons}
                                      setKeyPersons={setKeyPersons} key={keyPersons[i].id} mouID={id}/>
                );
            }
        }
        return sectionHtml;
    ...
    const generateDocumentSection = (arr) => {
        ...
        for (var i = 0; i < arr.length; i++) {
            if (attachments[i] === null) {
                sectionHtml.push(
                    <DocSection obj={null} index={i} attachments={attachments} setAttachments={setAttachments}
                                staffID={id} key={`temp${i}`} attachmentId={null}
                                updateAttachmentsHTTP={updateAttachmentsHTTP} url="moumoa"/>
                );
            } else {
                sectionHtml.push(
                    <DocSection obj={attachments[i]} index={i} attachments={attachments} setAttachments={setAttachments}
                                key={attachments[i].id} staffID={id} attachmentId={attachments[i].id}
                                updateAttachmentsHTTP={updateAttachmentsHTTP} url="moumoa"/>
                );
            }
        }
