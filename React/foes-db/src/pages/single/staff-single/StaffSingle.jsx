    ...
    const generateSection = (arr) => {
        ...
        for (var i = 0; i < arr.length; i++) {
            if (attachments[i] === null) {
                sectionHtml.push(
                    <FileSection obj={null} index={i} attachments={attachments} setAttachments={setAttachments}
                                 staffID={id} key={`temp${i}`} attachmentId={null}
                                 updateAttachmentsHTTP={updateAttachmentsHTTP}/>
                );
            } else {
                sectionHtml.push(
                    <FileSection obj={attachments[i]} index={i} attachments={attachments}
                                 setAttachments={setAttachments} key={attachments[i].id} staffID={id}
                                 attachmentId={attachments[i].id} updateAttachmentsHTTP={updateAttachmentsHTTP}/>
                );
            }
        }
        return sectionHtml;
